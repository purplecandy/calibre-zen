...
class Develop(Command):
    ...
    def add_postinstall_options(self, parser):
        parser.add_option('--make-errors-fatal', action='store_true', default=False, dest='fatal_errors', help='If set die on post install errors.')
        parser.add_option(
            '--no-postinstall',
            action='store_false',
            dest='postinstall',
            default=True,
            help="Don't run post install actions like creating MAN pages, setting" + ' up desktop integration and so on',
        )

    def add_options(self, parser):
        parser.add_option('--prefix', help='Binaries will be installed in <prefix>/bin')
        parser.add_option('--system-plugins-location', help='Path to a directory from which the installed calibre will load plugins')
    ...
    def consolidate_paths(self):
        ...
        for x in ('prefix', 'libdir', 'bindir', 'sharedir', 'staging_root', 'staging_libdir', 'staging_bindir', 'staging_sharedir'):
    ...
    def install_env_module(self):
        ...
        libdir = os.path.join(
            self.opts.staging_root,
            sysconfig.get_config_var('PLATLIBDIR') or 'lib',
    ...
    def run_postinstall(self):
        if self.opts.postinstall:
            ...
            PostInstall(self.opts, info=self.info, warn=self.warn, manifest=self.manifest)
    ...
    def write_templates(self):
        for typ in ('console', 'gui'):
            for name, mod, func in zip(basenames[typ], modules[typ], functions[typ]):
...
class Install(Develop):
    ...
    def add_options(self, parser):
        ...
        parser.add_option('--libdir', help='Where to put calibre library files. Default is <prefix>/lib')
        parser.add_option('--bindir', help='Where to put the calibre binaries. Default is <prefix>/bin')
        parser.add_option('--sharedir', help='Where to put the calibre data files. Default is <prefix>/share')
        parser.add_option(
            '--staging-root',
            '--root',
            default=None,
            help=(
                'Use a different installation root (mainly for packaging).'
        ...
        parser.add_option('--staging-libdir', help='Where to put calibre library files. Default is <root>/lib')
        parser.add_option('--staging-bindir', help='Where to put the calibre binaries. Default is <root>/bin')
        parser.add_option('--staging-sharedir', help='Where to put the calibre data files. Default is <root>/share')
        parser.add_option('--system-plugins-location', help='Path to a directory from which the installed calibre will load plugins')
        self.add_postinstall_options(parser)
    ...
    def install_files(self):
        ...
        for x in os.walk(self.SRC):
            ...
            for f in x[-1]:
                if os.path.splitext(f)[1] in ('.py', '.so', '.qss', '.svg'):
...
class Bootstrap(Command):
    ...
    def add_options(self, parser):
        ...
        parser.add_option(
            '--path-to-translations', help='Path to existing out-of-tree translations checkout. Use this to avoid downloading translations at all.'
        )

    def pre_sub_commands(self, opts):
        tdir = self.j(self.d(self.SRC), 'translations')
        clone_cmd = ['git', 'clone', f'https://github.com/{self.TRANSLATIONS_REPO}.git', 'translations']
